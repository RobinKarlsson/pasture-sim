...
public class Sheep extends Animal implements Entity {
	 ...
	 */
	public Sheep(Pasture pasture, int moveDelay, int timeWithoutFood, int breedDelay, int viewDistance) {
		super(pasture, moveDelay, breedDelay, timeWithoutFood, viewDistance, "sheep.gif");
	}
	...
	public void tick() {
		...
		Entity grass = findAt(this.pasture.getPosition(this), Grass.class);
		if(grass != null)
			eat(grass);
	...
	private double evaluatePosition(List<Entity> surroundingEntitys, Point positionToEval) {
		double closestGrass = distanceToClosest(surroundingEntitys, Grass.class, positionToEval);
		double closestWolf = distanceToClosest(surroundingEntitys, Wolf.class, positionToEval);
		double distanceOldPos = distanceOldPosition(positionToEval);
		return 100 * closestWolf - closestGrass + 0.01 * distanceOldPos;
	}
	...
	public boolean isCompatible(Entity other) {
        if(other instanceof Sheep || other instanceof Fence) {
			return false;
        }
        return true;
    }
	...
	/**
     * create a new Sheep object if theres a free neighbouring Point for the child to occupy
     */
	private void breed() {
